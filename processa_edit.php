...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($id > 0 && $nome && $nascimento && $email && $aluno_estacio && $genero) {
        $stmt = $conn->prepare("UPDATE inscricoes SET nome=?, nascimento=?, email=?, bairro=?, aluno_estacio=?, comorbidade=?, genero=? WHERE id=?");
        $stmt->bind_param("sssssssi", $nome, $nascimento, $email, $bairro, $aluno_estacio, $comorbidade, $genero, $id);
